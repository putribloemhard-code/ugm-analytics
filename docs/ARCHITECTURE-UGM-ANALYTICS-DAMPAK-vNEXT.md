...
| **Laporan dampak berbab (2026-09-23)** | Blok "Laporan dampak per bab" di scene `dampak` (`/dampak`): Daftar isi + BAB II Sosial (4 tema) → BAB III Ekonomi (5) → BAB IV Lingkungan (5) mengikuti daftar isi resmi `sumber/LAPORAN DAMPAK SOSIAL, EKONOMI, DAN LINGKUNGAN UGM 2025.pdf`; tiap sub-bab 2.1–4.5 = tema Kepmen + panel indikator resmi (definisi/kriteria/formula/satuan/SDGs). Data = payload `chapters` di `/analytics/story` (urutan: `CHAPTER_ORDER` di `services/story.py`; indikator dari `kepmen_sdg.py`); render `web/src/laporan.tsx` + CSS `laporan-*` | `api/app/services/story.py`, `web/src/laporan.tsx` |
