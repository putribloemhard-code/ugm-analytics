...
- State runtime disimpan di `../runtime/` (di luar repo, gitignored): `runtime/postgres`,
  `runtime/mysql-reader-v2`, `runtime/accreditation-uploads`, `runtime/accreditation-generated`
  (laporan Word akreditasi, dipakai untuk unduh ulang dari Profil). Ia dimulai kosong dan tidak menyentuh
  data stack lain.
...
Skema dibuat dari `information_schema` MySQL (sejak 2026-09-23): tipe kolom yang benar
(`tinyint(1)` → `boolean`, `datetime` → `timestamp`, ...), NOT NULL, default, PRIMARY KEY, UNIQUE,
dan `AUTO_INCREMENT` → `GENERATED BY DEFAULT AS IDENTITY` (sequence disetel ke `MAX(id)+1`).
Versi lama memakai `pandas.to_sql` saja sehingga di PostgreSQL tidak ada PK/UNIQUE/IDENTITY dan
kolom boolean menjadi BIGINT — registrasi, login, upload, dan simpan data akreditasi gagal.
...
`berita_sitemap_sdg(url)`, `berita_unit_kerja(url)`.

**Server yang sudah dimigrasi dengan skrip lama — WAJIB sekali sebelum memakai versi ini**
(tanpa ini login pun gagal, karena rate limit login menulis boolean ke kolom yang masih BIGINT):

    docker compose -f deploy/compose.yml run --rm --entrypoint python api \
      /app/migrate_mysql_to_postgres.py --perbaiki-skema

Mode ini tidak menyalin ulang data: mengubah tipe kolom di tempat, memberi nomor pada baris ber-`id`
NULL (hasil INSERT di server lama), lalu memasang PK/UNIQUE/IDENTITY. Aman diulang (idempoten).
Kolom yang seharusnya NOT NULL tetapi masih berisi NULL tidak dipaksa — skrip mencetak
`PERINGATAN` untuk diperiksa manual.

## Ekstraksi AI dokumen akreditasi (opsional)

Tanpa konfigurasi, tombol "Ekstrak data" di `/akreditasi` nonaktif. Untuk mengaktifkan:
`cp deploy/openai.env.example deploy/openai.env && chmod 600 deploy/openai.env`, isi
`OPENAI_API_KEY` (dan `OPENAI_BASE_URL` bila memakai gateway), lalu `docker compose up -d api`.
