"""Siapkan file logo untuk web: rapikan gambar mentah lalu simpan ke
web/public/logo/ dengan nama yang dipakai kode (assetUrl('logo/...')).

CARA PAKAI (dari folder root project, D:\\ugm-analytics):

    venv\\Scripts\\python.exe web\\siapkan_logo.py sumber\\picture\\dampak.png dampak.png

Argumen: <file mentah> <nama file tujuan di web/public/logo/>
...
kotak putih kecil ("chip"), baik di header, kartu rute, maupun judul
...
DIR_LOGO = Path(__file__).resolve().parent / "public" / "logo"
...
def main() -> None:
    ...
    p.add_argument("nama", help="nama file tujuan di web/public/logo/ (mis. dampak.png)")
